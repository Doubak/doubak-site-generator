 ...
 * 不筛选——`bin/deploy.js` 把三栏原样印出来，让按下去的人自己看见。
 *
 * **站点这一步与 NeoDB 导出那一步的默认值不一样，而那不是不一致。** 那边「不公开」=
 * 记录照样在你账号里、只是不对外，所以作者自己藏的可以安全地收成 `visibility=2`；
 * 这边「不发出去」= 这一页在网上根本不存在，没有一个同样安全的动作，所以默认全发、
 * 由 `--drop-notes` 明说。两边对**豆瓣锁掉的**那一类处置是一致的：照常公开。这是那条
