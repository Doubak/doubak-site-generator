 ...
 * 广播出处那个链接的 `title`。
 *
 * **它是主题认出这一段的判据**（见 `broadcastBlock` 里那段注释，以及骨架
 * `site.css` 里 `a[href^="http"][title]` 那条规则），所以是一个具名常量而不是
 * 一句就地写死的字符串：改这里的措辞不该改变版式，而删掉它会让角标退回成
 * 独占一行的文字——两边各有一条测试钉着。
 */
export const SOURCE_LINK_TITLE = '在豆瓣上看这条广播';
...
export function broadcastBlock(b, { images = {}, covers = {}, linkPrefix = '../' } = {}) {
  ...
  // 不许留空」是同一条。所以文字照写，交给主题把它收成时间戳右端的一枚角标——
  // **版式是主题的事**，正文里不塞 HTML（塞了就得开 unsafe）。
  //
  // 与下面两处「绝不回退到豆瓣的 URL」不矛盾：那两条说的是**内容**（接不回本地的
  // 作品名、全文不许变成站外链接），这一条是**出处**，与作品页的「豆瓣原页」同一件事。
  //
  // ## 那个 title 是给主题认的，不是装饰
  //
  // 主题要把这一行收成角标，就得先认出「哪一段是出处」。原来认的是位置加形状：
  // 「时间戳标题紧跟着的、整段只有一个站外链接的那一段」。**那是个代理**——它与
  // 「这一段是出处」只在「没有别的段落长这样」的时候等价，而这两件事随时会分家：
  //
  //   实测这份档案：3503 条广播里 10 条没有出处（`data-status-url` 没抽到），
  //   同时站点上有 3 段正文**整段只是一个裸链接**（Goldmark 的 linkify 会把它
  //   自动变成 `<a>`）。今天这两组没撞上（撞车 0 处），撞上的那天，用户自己写的
  //   那句话会被当成出处**收成一枚图标**——正文静默消失，这个项目最怕的那类。
  //
  // 所以给它一个 Markdown 表达得出来的标记。`title` 是 CommonMark 核心语法，
  // 换任何 SSG 都渲染成 `title=`，而 linkify 自动生成的链接**永远没有 title**，
  // 判据因此是精确的而不是巧合的。它同时是个真功能：这个链接的文字在视觉上被
  // 收掉了，鼠标停上去总得说得出它去哪儿。
  const stamp = b.postedAtRaw ?? b.postedAt ?? '时间未知';
  const out = [`### ${stamp}`, ''];
  if (b.url) out.push(`[豆瓣原页](${b.url} "${SOURCE_LINK_TITLE}")`, '');
  // **只有作者本人看得见的那些，页面上要说出来。**
  //
  // 走到这里说明生成时加了 `--include-private`——默认那条路在 `generate()` 入口
  // 就把它们筛掉了。发出去时必须标出来，理由是这一条正是整件事的起因：豆瓣发一篇
  // 私密日记时会同步一条广播，正文一字不差，而它**绕过日记那一侧的所有开关**。
  // 一个看起来和别人一样的公开条目，实际上是从只有你自己看得见的时间线上来的
  // ——不说，用户就不知道自己刚发出去了什么。
  //
  // 写进正文而不是只写进 front matter：Markdown 是产品，HTML 只是它的一个消费者，
  // 只写 front matter 的话这件事在纯 Markdown 那棵树里根本不存在。
  //
  // **只标 `private`，不标 `null`。** null 有两个成因，而其中一个是「这份
  // canonical 是 0.13.0 之前生成的」——那时候**每一条**广播都是 null，逐条挂锁
  // 就是给整个站点每条广播都加一枚图标，「一份永远有标记的页面等于没有标记」。
  // 读不出来的那一条另有去处：默认它根本不会被发出去，而部署预演会逐条点名。
  if (b.visibility === 'private') {
    out.push('🔒 **这条广播在豆瓣上只有你自己看得见。**', '');
  }
